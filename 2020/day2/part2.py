...
def validate_passwords(data):
    valid = 0
    for element in data:
        if element[3].__len__() >= element[0] and element[3].__len__() >= element[1]:
            if element[3][element[0] - 1] == element[2] and not element[3][element[1] - 1] == element[2] or not \
            element[3][element[0] - 1] == element[2] and element[3][element[1] - 1] == element[2]:
                valid += 1
